...
import { TrendingUp, TrendingDown, DollarSign, Trash2, Calendar, LayoutGrid, Activity } from 'lucide-react';
...
export function Reports({ products, customers, transactions, settings }: ReportsProps) {
  ...
  const filteredTxs = transactions.filter(t => new Date(t.createdAt) >= cutoffDate);
  ...
  const totalSales = filteredTxs.filter(t => t.type === 'sale').reduce((s, t) => s + t.amount, 0);
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 overflow-hidden shadow-sm">
        ...
        <div className="divide-y divide-slate-50 dark:divide-slate-800 max-h-80 overflow-y-auto no-scrollbar">
          {filteredTxs.slice(0, 50).map(t => (
            <div key={t.id} className="p-4 flex justify-between items-center hover:bg-slate-50 dark:hover:bg-slate-800/50 group transition-colors">
